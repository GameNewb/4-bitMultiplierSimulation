...
public class MultiplierSimulation 
{
    ...
    public static short multiply(byte multiplicand, byte multiplier)
    {
        ...
        for(int i = 0; i < 4; i++) //Condition to check if multiplier != 0 based on the hardware protocol
        {
            if ((multiplier & 1) != 0) //Logical AND
            {
                ...
                //Ends the infinite loop for trivial case of multiplier being always -1 after shifting
                //if(multiplier == -1) 
                //{
                //    product = (short) (product * -1); //Corrects some output
                //    break;
                //}
            } 
        }
